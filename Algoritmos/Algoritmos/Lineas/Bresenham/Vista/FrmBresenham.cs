...
using GeometriaComputacional.Lineas.Bresenham.Controlador;
...
namespace GeometriaComputacional
{
    ...
    {
        private readonly BresenhamController controlador = new();
        private PuntoLinea? p1;
        private PuntoLinea? p2;
        private PuntoLinea? puntoCapturado;
        private ResultadoBresenham? resultado;
        ...
        private void ActualizarResumen()
        {
            ...
            lblFormulaResumen.Text = $"|Delta X| = {Math.Abs(resultado.DeltaX)}   |Delta Y| = {Math.Abs(resultado.DeltaY)}   error inicial = dx - dy. " +
                                     "En cada paso se evalua 2 * error para decidir si avanza X, Y o ambos.";
        }

        private void LlenarTablaBresenham()
        {
            dataGridView1.Rows.Clear();
            ...
            {
                PasoBresenham paso = resultado.Pasos[i];
        ...
        private void DibujarRectaBresenham(Graphics g)
        {
            if (p1 == null)
            {
                DibujarAyuda(g);
        ...
        private void DibujarLeyenda(Graphics g)
        {
            ...
            DibujarItemLeyenda(g, caja.X + 136, caja.Y + 34, Color.FromArgb(220, 38, 38), "Pixel Bres.");
